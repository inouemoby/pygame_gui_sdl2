...
from pygame_gui_sdl2.core.text.text_layout_rect import TextFloatPosition
...
class TextBoxLayoutRow(pygame.Rect):
    ...
    def __init__(self, renderer: Renderer, row_start_x, row_start_y, row_index, line_spacing, layout):
    ...
    def finalise(self, texture: TextureLayer, current_end_pos: Optional[int] = None,
                    ...
                    if isinstance(text_chunk, TextLineChunkFTFont):
                        if current_end_pos is not None and cumulative_letter_count is not None:
                            if cumulative_letter_count < current_end_pos:
                                text_chunk.finalise(texture, chunk_view_rect, self.y_origin,
    ...
    def clear(self):
        ...
        """
        if self.target_texture is not None and self.texture_row_dirty:
