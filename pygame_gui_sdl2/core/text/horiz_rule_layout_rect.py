...
class HorizRuleLayoutRect(TextLayoutRect):
    ...
    def __init__(self, renderer: Renderer,height: int,
                 colour_or_gradient: Color,
                 rule_dimensions: Tuple[int, int] = (-1, 1),
                 has_shade: bool = True,
                 alignment=ALIGN_CENTER):
        super().__init__(renderer, dimensions=(-1, height), should_span=True)
    ...
    def finalise(self, target_texture: TextureLayer,
                 target_area: Rect,
        ...
        if self.shade:
            self._draw_shaded_rule(x_start, y_start, target_texture)
        else:  # no shade
            target_texture.draw_rect(color=self.colour_or_gradient,
                             rect=Rect((x_start, y_start),
                                       (self.rule_dimensions[0], self.rule_dimensions[1])))

    def _draw_shaded_rule(self, x_start, y_start, target_texture: TextureLayer):
        """
        ...
        target_texture.draw_line(color=self.colour_or_gradient,
                         start_pos=top_left,
                         end_pos=top_right)
        if self.rule_dimensions[1] > 1:
            target_texture.draw_line(color=self.med_shade_colour,
                             start_pos=top_left,
                             end_pos=bottom_left)
        target_texture.draw_line(color=self.light_shade_colour,
