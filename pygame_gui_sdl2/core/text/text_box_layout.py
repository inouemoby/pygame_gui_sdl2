...
from pygame_gui_sdl2.core.text.hyperlink_text_chunk import HyperlinkTextChunk
from pygame_gui_sdl2.core.text.text_line_chunk import TextLineChunkFTFont
from pygame_gui_sdl2.core.text.image_layout_rect import ImageLayoutRect

from pygame_gui_sdl2.core.text.text_box_layout_row import TextBoxLayoutRow
...
class TextBoxLayout:
    ...
    def __init__(self, renderer: Renderer,
        ...
        self.alpha = 255
        self.pre_alpha_final_texture = None  # only need this if we apply non-255 alpha

        self.layout_rect_queue = self.input_data_rect_queue.copy()
        current_row = TextBoxLayoutRow(self.renderer, row_start_x=self.layout_rect.x,
        ...
        if len(self.layout_rect_queue) > 0:
            ...
            empty_text_chunk = TextLineChunkFTFont(self.renderer,
                                                   "",
                                                   self.default_font,
                                                   False,
    ...
    def _add_row_to_layout(self, current_row: TextBoxLayoutRow, last_row=False):
        ...
        if len(current_row.items) == 0 and not last_row:
            current_row.add_item(LineBreakLayoutRect(self.renderer, dimensions=(2, self.last_row_height),
    ...
    def finalise_to_texture(self, texture: TextureLayer):
        """
        ...
        if self.current_end_pos != self.letter_count:
            ...
            for row in self.layout_rows:
                cumulative_letter_count = row.finalise(texture,
                                                       self.current_end_pos,
                                                       cumulative_letter_count)
        else:
            for row in self.layout_rows:
                row.finalise(texture)
    ...
    def set_alpha(self, alpha: int):
        ...
        if self.alpha == 255 and alpha != 255:
            if self.finalised_texture is not None:
                self.pre_alpha_final_texture = self.finalised_texture.copy()

        self.alpha = alpha

        if self.pre_alpha_final_texture is not None:
    ...
    def set_text_selection(self, start_index, end_index):
        ...
        if self.finalised_texture is not None:
            for row in rows_to_finalise:
                row.finalise(self.finalised_texture)
