...
class ShadowGenerator:
    ...
    def create_shadow_corners(self,
                              ...
                              aa_amount=4) -> Dict[str, TextureLayer]:
        """
        ...
        if sub_radius > 0:
            corner_sub_texture = TextureLayer(self.renderer, size=corner_rect.size)
            corner_sub_texture.fill(pygame.Color('#00000000'))

            corner_sub_texture.draw_circle(pygame.Color('#FFFFFFFF'),
                               corner_rect.size,
                               sub_radius)

            corner_small_sub_texture = corner_sub_texture.copy().scale_to((corner_radius_param,
                                                                     corner_radius_param))

            tl_corner.extend(corner_small_sub_texture, dest=(0, 0))
    ...
    def _create_single_corner_and_edge(aa_amount, corner_radius_param, corner_rect,
        ...
        """
        final_corner_texture = TextureLayer(global_renderer, size=(corner_radius_param * aa_amount,
                                                       corner_radius_param * aa_amount))
        final_corner_texture.fill(pygame.Color('#00000000'))
        final_edge_texture = TextureLayer(global_renderer, size=(shadow_width_param * aa_amount,
        ...
        for _ in range(shadow_width_param):
            if corner_rect.width > 0 and corner_rect.height > 0 and corner_radius > 0:
                # Edge
                edge_shadow_texture = TextureLayer(global_renderer, size=edge_rect.size)
                edge_shadow_texture.fill(pygame.Color('#00000000'))
                edge_shadow_texture.fill(pygame.Color(0, 0, 0, int(shadow_alpha)),
                                         pygame.Rect(0,
                                                     edge_rect.height - edge_shadow_fade_height,
                                                     edge_rect.width,
                                                     edge_shadow_fade_height))

                final_edge_texture.extend(edge_shadow_texture, dest=(0, 0))

                # corner
                corner_shadow_texture = TextureLayer(global_renderer, size=corner_rect.size)
                corner_shadow_texture.fill(pygame.Color('#00000000'))
                corner_shadow_texture.draw_circle(pygame.Color(0, 0, 0, int(shadow_alpha)),
                                   corner_centre,
                                   corner_radius)

                final_corner_texture.extend(corner_shadow_texture, dest=(0, 0))
    ...
    def create_new_rectangle_shadow(self,
        ...
        final_texture = TextureLayer(self.renderer, size=(width, height))
        final_texture.fill(pygame.Color('#00000000'))
        ...
        final_texture.extend(edges_and_corners["top_left"], dest=(0, 0))
        final_texture.extend(edges_and_corners["top_right"], dest=(width - corner_radius_param, 0))

        final_texture.extend(edges_and_corners["bottom_left"],
                           dest=(0, height - corner_radius_param))
        final_texture.extend(edges_and_corners["bottom_right"],
                           dest=(width - corner_radius_param, height - corner_radius_param))
        ...
        if width - (2 * corner_radius_param) > 0:
            ...
            bottom_edge = edges_and_corners["bottom"].copy().scale_to(
                                                 (width - (2 * corner_radius_param),
                                                  shadow_width_param))
            final_texture.extend(top_edge, dest=(corner_radius_param, 0))
            final_texture.extend(bottom_edge, dest=(corner_radius_param, height - shadow_width_param))
        ...
        if height - (2 * corner_radius_param) > 0:
            ...
            final_texture.extend(left_edge, dest=(0, corner_radius_param))
            final_texture.extend(right_edge, dest=(width - shadow_width_param,
                                            corner_radius_param))

        self.short_term_rect_cache[shadow_id] = final_texture
        ...
        for i in range(shadow_width_param):
            if shadow_width > 0 and shadow_height > 0:
                shadow_rect = pygame.Rect(i * aa_amount,
                                          ...
                                          shadow_width,
                                          shadow_height)
                shadow_texture.draw_ellipse(pygame.Color(0, 0, 0, shadow_alpha), shadow_rect)
                shadow_width -= (2 * aa_amount)
                shadow_height -= (2 * aa_amount)
                shadow_alpha += alpha_increment

        final_texture = shadow_texture.copy().scale_to((width, height))
        self.created_ellipse_shadows[(str(width) +
                                      'x' +
                                      str(height) +
                                      'x' +
                                      str(shadow_width_param))] = final_texture
        return final_texture
