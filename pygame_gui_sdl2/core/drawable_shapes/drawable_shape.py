...
class DrawableShapeState:
    ...
    def get_texture(self) -> TextureLayer:
        if self.transition is not None:
            return self.transition.produce_blended_result_texture()
        else:
            return self.texture
...
class DrawableStateTransition:
    ...
    def produce_blended_result_texture(self) -> TextureLayer:
        ...
        """
        # 缺少混合texture的方法，只能简单返回和目标颜色乘算
        result = self.states[self.start_stat_id].texture.copy()
        blended_target = self.states[self.target_state_id].texture.copy()
        # start_multiply_texture = pygame._sdl2.Texture(self.states[self.start_stat_id].renderer,
        #     self.states[self.start_stat_id].texture.get_rect().size)
        # target_multiply_texture = start_multiply_texture.copy()
...
class DrawableShape:
    ...
    """
    def __init__(self,
                 renderer: pygame._sdl2.Renderer,
        ...
        self.states = {}
        for state in states:
            self.states[state] = DrawableShapeState(self.renderer, state)
        ...
        self.base_texture: Optional[TextureLayer] = None
    ...
    def get_active_state_texture(self) -> TextureLayer:
        if self.active_state is not None:
            return self.active_state.get_texture()
        else:
    ...
    def get_texture(self, state_name: str) -> TextureLayer:
        if state_name in self.states and self.states[state_name].texture is not None:
            return self.states[state_name].texture
        elif state_name in self.states and self.states['normal'].texture is not None:
            return self.states['normal'].texture
        else:
            empty_texture = TextureLayer(self.renderer, size=(0, 0))
    ...
    def get_fresh_texture(self) -> TextureLayer:
        ...
        return self.get_active_state_texture()
    ...
    def has_fresh_texture(self) -> bool:
        return self.active_state.has_fresh_texture
    ...
    def build_text_layout(self):
        ...
        if 'text' in self.theming and 'font' in self.theming and self.theming['text'] is not None:
            ...
            default_font_data = {"font": self.theming['font'],
                                 ...
                                 "bg_colour": pygame.Color('#00000000')}
            self.text_box_layout = TextBoxLayout(self.renderer, deque([text_chunk]), text_actual_area_rect,
