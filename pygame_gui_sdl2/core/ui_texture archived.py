...
class TextureLayer:
    def __init__(self, renderer: Renderer, size: Tuple[int, int] = None, texture: Texture = None, surface: Surface= None, maxlen: int=15):
        ...
        if not size is None:
            if size[0] <= 0 or size[1] <= 0:
                self.father_texture = Texture(renderer=renderer, size=(1,1), scale_quality=0)
                # self.father_texture.alpha = 0
            else:
                self.father_texture = Texture(renderer=renderer, size=size, scale_quality=2)
    ...
    def append(self, texture: Texture, srcrect: Rect=None, dstrect: Rect=None, dest: Union[Tuple, Rect] = None, area: Rect = None, angle=0, flip_x=False, flip_y=False, color = Color('#FFFFFFFF'), can_override=True, can_clean=False, special_flags=0):
        if texture is None:
            return
        if srcrect is None:
    ...
    def extend(self, texture_layer, srcrect: Rect=None, dstrect: Rect=None, dest: Union[Tuple, Rect] = None, area: Rect = None, angle=0, flip_x=False, flip_y=False, color = Color('#FFFFFFFF'), can_override=False, can_clean=False):
        if isinstance(texture_layer, TextureLayer):
            ...
            if self.check_if_empty(oldfather_texture_data):
                ...
                new_oldfather_texture_data = self.blend_to_father(oldfather_texture_data, srcrect, dstrect, angle, flip_x, flip_y, color)
            ...
            while texture_layer.texture_list:
                texture_data = texture_layer.texture_list.popleft()
                if self.check_if_empty(texture_data):
                    continue
                new_texture_data = self.blend_to_father(texture_data, srcrect, dstrect, angle, flip_x, flip_y, color)
            ...
            self.append(texture_layer, srcrect, dstrect, dest, area, angle, flip_x, flip_y, color, can_override, can_clean)
            
        elif isinstance(texture_layer, Surface):
            texture = Texture.from_surface(self.father_texture.renderer, texture_layer)
            self.append(texture, srcrect, dstrect, dest, area, angle, flip_x, flip_y, color, can_override, can_clean)
        elif isinstance(texture_layer, list[TextureLayer]):
            for texture_data in texture_layer:
                self.extend(texture_data, srcrect, dstrect, dest, area, angle, flip_x, flip_y, color, can_override, can_clean)
            
    def blend_to_father(self, texture_data: dict, adjust_srcrect: Rect=None, adjust_dstrect: Union[Tuple, Rect] = None, adjust_angle=0, adjust_flip_x=False, adjust_flip_y=False, adjust_color=Color('#FFFFFFFF')):
        new_texture_data = {}
    ...
    def draw(self, srcrect: Rect=None, dstrect: Rect=None, dest: Union[Tuple, Rect] = None, area: Rect = None, angle=0, flip_x=False, flip_y=False):
        ...
        if self.check_if_empty(new_father_texture_data):
            ...
            self.father_texture.color = self.father_color.rgb
        ...
        for texture_data in self.texture_list:
            ...
            if self.check_if_empty(new_texture_data):
                ...
                texture.color = texture_data["color"].rgb
    ...
    def fill(self, color, rect: Rect=None, special_flags=0):
        ...
        if color.a >= 255 or color.a <= 0:
            if rect is None or rect == self.father_dstrect:
                self.refresh(self._color_texture)
                self.father_color = color
                return self
        ...
        if rect is None:
            ...
            dstrect = rect
        self.extend(self._color_texture, dstrect=dstrect, color=color)
        return self
    ...
    def subtexture(self, rect: Rect):
        ...
        subtexture.father_color = self.father_color.rgb
