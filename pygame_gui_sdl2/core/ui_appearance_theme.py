...
from pygame_gui_sdl2.core.interfaces.appearance_theme_interface import IUIAppearanceThemeInterface
from pygame_gui_sdl2.core.utility import create_resource_path, PackageResource
from pygame_gui_sdl2.core.utility import ImageResource, SurfaceResource, TextureResource
...
from pygame_gui_sdl2.core.texture_cache import TextureCache
...
class UIAppearanceTheme(IUIAppearanceThemeInterface):
    ...
    def __init__(self, renderer, resource_loader: IResourceLoader, locale: str):
        ...
        self.font_dict = UIFontDictionary(self.renderer, self._resource_loader, locale)
        self.shadow_generator = ShadowGenerator(self.renderer)
        self.shape_cache = TextureCache(self.renderer)
        ...
        self.ele_font_res = {}
        self.ui_element_image_textures = {}
        self.ui_element_misc_data = {}
        self.image_resources = {}  # type: Dict[str,ImageResource]
        self.texture_resources = {}  # type: Dict[str,TextureResource]
    ...
    def _load_default_theme_file(self):
        ...
        """
        self.load_theme(PackageResource('pygame_gui_sdl2.data', 'default_theme.json'))
    ...
    def update_caching(self, time_delta: float):
        ...
        self.shape_cache.update(time_delta)
    ...
    def _load_images(self):
        ...
        for element_key, image_ids_dict in self.ui_element_image_locs.items():
            if element_key not in self.ui_element_image_textures:
                self.ui_element_image_textures[element_key] = {}
            for image_id in image_ids_dict:
                ...
                if image_resource_data['changed']:
                    ...
                    if image_resource is not None:
                        if 'sub_texture_rect' in image_resource_data:
                            texture_id = (image_resource.image_id +
                                          str(image_resource_data['sub_texture_rect']))
                            if texture_id in self.texture_resources:
                                ...
                                if self._resource_loader.started():
                                    error = texture_resource.load()
                                    if error is not None:
                                        warnings.warn(str(error))
                                else:
                                    self._resource_loader.add_resource(texture_resource)
                        else:
                            texture_id = image_resource.image_id
                            if texture_id in self.texture_resources:
                                texture_resource = self.texture_resources[texture_id]
                        ...
                        self.ui_element_image_textures[element_key][image_id] = texture_resource
    ...
    def get_image(self, image_id: str, combined_element_ids: List[str]) -> Texture:
        ...
        for combined_element_id in combined_element_ids:
            if (combined_element_id in self.ui_element_image_textures and
                    image_id in self.ui_element_image_textures[combined_element_id]):
                return self.ui_element_image_textures[combined_element_id][image_id].texture
    ...
    def _load_element_image_data_from_theme(self,
        ...
        for image_key in loaded_img_dict:
            ...
            if 'sub_texture_rect' in loaded_img_dict[image_key]:
                rect_list = str(loaded_img_dict[image_key]['sub_texture_rect']).strip().split(',')
                if len(rect_list) == 4:
                    try:
                        ...
                        warnings.warn("Unable to create subtexture rectangle from string: "
                                      "" + loaded_img_dict[image_key]['sub_texture_rect'])
                    else:
                        rect = pygame.Rect(top_left, size)

                    if ('sub_texture_rect' in img_res_dict and
                            rect != img_res_dict['sub_texture_rect']):
                        img_res_dict['changed'] = True
                    img_res_dict['sub_texture_rect'] = rect
