...
from pygame_gui_sdl2._constants import UI_TEXT_ENTRY_FINISHED, UI_TEXT_ENTRY_CHANGED, OldType

from pygame_gui_sdl2.core.interfaces import IContainerLikeInterface, IUIManagerInterface
from pygame_gui_sdl2.core.utility import clipboard_paste, clipboard_copy, translate
...
class UITextEntryLine(UIElement):
    ...
    def disable(self):
        ...
        if self.is_enabled:
            ...
            if self.drawable_shape is not None:
                ...
                self.background_and_border = self.drawable_shape.get_texture('disabled')
            self.edit_position = 0
            self.select_range = [0, 0]
            self.redraw()
    ...
    def enable(self):
        ...
        if not self.is_enabled:
            ...
            if self.drawable_shape is not None:
                ...
                self.background_and_border = self.drawable_shape.get_texture('normal')
            self.redraw()
    ...
    def apply_active_text_changes(self):
        self.drawable_shape.get_fresh_texture()
        if self.drawable_shape.text_box_layout.finalised_texture is not None:
            self.image.merge_text(self.drawable_shape.text_box_layout.finalised_texture)